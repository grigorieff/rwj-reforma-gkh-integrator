...
function create_table_rwj_reforma_gkh_integrator_house_profile_data_988()
{
	...
	$sql = "
				CREATE TABLE ".$table_house_profile_data_988." (
				  ...
				  `last_update` TIMESTAMP NOT NULL COMMENT 'Последнее изменение анкеты',
				  `last_task_update` TIMESTAMP NOT NULL COMMENT 'Последнее обновление данных',
					...
					{	
						...
						foreach ($management_contract_files as $file_id)
						{
							...
							if (FileIdToDataBaseExists((int)$file_id) == 0){
								insert_table_rwj_reforma_gkh_integrator_files(array('id' => (int)$file_id, 'need_update' => 0));
							}
						}
					}
					...
					if (HouseToDataBaseExist($House->house_id, $Period->state) == 0) {
						...
						$wpdb->insert (
							...
							array( 
								...
								'last_update'			=>	esc_sql($HouseProfile988->last_update), 
								'last_task_update'		=>	esc_sql($last_task_update->format('Y-m-d H:i:s')),
						...
						if ($HouseProfile988->last_update != get_last_update_house_profile($HouseProfile988->house_id, $Period->state)) {
							$wpdb->update(
								$house_profile_988,
								array(
									'period_id'				=>	esc_sql($Period->id), 
							...
							set_null_need_update($HouseProfile988->house_id);
						}
					}
				}
			}
...
function insert_table_rwj_reforma_gkh_integrator_files($element)
{
	global $wpdb;
	$table_files = $wpdb->prefix.'rwj_reforma_gkh_integrator_files';
